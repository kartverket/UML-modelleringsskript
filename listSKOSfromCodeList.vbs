...
' script:			listSKOSfromCodeList
' description:		Write a skos-file containing the codes and elements of a CodeList class
' author:			Kent
...
' date  :			2025-09-03 stripped old script to output only one SKOS-file
...
sub listCodesForChodenCodeList()
...
'			message = "--------------------------------------------------------------------------------------" & vbCrLf & vbCrLf & _
			...
			message = message & "Script listSKOSfromCodeList versjon 2025-09-03 (Kent Jonsrud)" & vbCrLf & vbCrLf
			message = message & "Creates one SKOS/RDF/xml format file with all codes, "
			message = message & "file is created in the same folder as the project file."& vbCrLf  & vbCrLf
'			message = message & "--------------------------------------------------------------------------------------"
			dim box
		...
		'		end if
				if namespace = "" then
					namespace = "http://iso.gom.no/" & theElement.Name & "/"
				end if
...
sub listCodelistCodes(el,namespace)
    ...
    objFile.Write"    <skos:prefLabel xml:lang=""en"">"&utf8(presentasjonsnavn)&"</skos:prefLabel>" & vbCrLf
    objFile.Write"    <skos:definition xml:lang=""en"">"&utf8(getCleanDefinitionText(el))&"</skos:definition>" & vbCrLf
    objFile.Write"  </skos:ConceptScheme>" & vbCrLf

	dim attr as EA.Attribute
	for each attr in el.Attributes
		call listSKOSfromCode(attr,el.Name,namespace)
	next
...
Sub listSKOSfromCode(attr, codelist, namespace)
	...
	if presentasjonsnavn = "" then presentasjonsnavn = toLabel(attr.Name)
		...
		objFile.Write"    <skos:definition xml:lang=""en"">"&utf8(getCleanDefinitionText(attr))&"</skos:definition>" & vbCrLf
