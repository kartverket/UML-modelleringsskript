...
' skriptnavn:         settPrefiskPåKoder

sub fixCodelists(el,prefiks)
...
sub oppdaterKoderForEnValgtKodeliste()
	...
	if not theElement is nothing  then
		if theElement.Type="Class" and theElement.Stereotype = "codeList" Or theElement.Stereotype = "CodeList" Or theElement.Stereotype = "enumeration" then
			...
			'inputBoxGUI to receive user input regarding the prefix
			dim prefiks
			prefiks = InputBox("Please select the prefix.", "prefix", "code")

			call fixCodelists(theElement,prefiks)
    Else
