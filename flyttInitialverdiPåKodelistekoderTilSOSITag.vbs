...
!INC Local Scripts.EAConstants-VBScript
 ...
 ' Script Name: flyttInitialverdiPåKodelistekoderTilSOSITag 
 ' Author: Kent Jonsrud Kartverket
 ' Date: 2016-09-09 
 ' Purpose: moves initial values on codes in a code list to tagged values SOSI_verdi, must be checked afterwards!

sub fixOldCodelists(el)
	Repository.WriteOutput "Script", Now & " Script flyttInitialverdiPåKodelistekoderTilSOSITag running on CodeList: " & el.Name, 0
...
Sub flyttInitialverdiTilTagSOSI_verdi(attr)
		If attr.Default <> "" then
  		Repository.WriteOutput "Script", "Code: "&attr.Name&" Initial value to be moved: " & attr.Default,0
...
sub oppdaterKoderForEnValgtKodeliste()
	...
	if not theElement is nothing  then
		if (theElement.ObjectType = otElement) then
			dim box, mess
			mess = 	"Moves initial values on codes in a code list to tagged values SOSI_verdi. Script version 2016-09-09." & vbCrLf
