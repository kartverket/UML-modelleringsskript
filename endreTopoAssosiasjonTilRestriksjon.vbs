...
' Date: 		06.04.2016
' Date: 		2017-10-09 Added Ok-button (and added a refresh on line 235), Kent Jonsrud
' Navn:         endreTopoAssosiasjonTilRestriksjon
' Formål:       Fjerner topo-assosiasjoner og legger inn en restriksjon KanAvgrensesAv.
'
...
'					
		case otPackage
			...
			if UCase(thePackage.element.stereotype) = UCase("applicationSchema") then
				...
				dim box, mess
				mess = 	"Fjerner alle assosiasjoner med stereotype «topo»,"
				mess = mess + " og hvis den ene klassen har flategeometri og navigerbarhet til den andre klassen"
				mess = mess + " og den andre klassen har kurvegeometri" 
				mess = mess + " legges det inn en restriksjon KanAvgrensesAv med navnet på den andre klassen."& vbCrLf
				mess = mess + "Skript versjon 2016-04-06." & vbCrLf
				mess = mess + "Advarsel! Dette skriptet vil kunne endre på alle assosiasjonene i hele pakka: "& vbCrLf & "[«" & thePackage.element.Stereotype & "» " & thePackage.Name & "]."

				box = Msgbox (mess, vbOKCancel, "Fjerner alle assosiasjoner med stereotype «topo».")
				select case box
				case vbOK
...
'			set theMethod = Repository.GetTreeSelectedObject()
		...
		case else
			' Error message
			Session.Prompt "This script does not support items of this type. Please choose a package with stereotype applicationSchema in order to start the script.", promptOK
						...
						if UCase(currentConnector.Stereotype) = UCase("topo") then
							...
							if hasFlateAttributeType and (((oppositeSideNavigable = "Navigable") and not (currentElementSideNavigable = "Navigable")) or ((currentElementSharedAggregation = true) and (oppositeSideSharedAggregation = false))) then
								...
								for constraintsCounter = 0 to currentElementConstraints.Count - 1
									'Session.Output("Inspecting topo association number " & constraintsCounter & " of " & currentElementConstraints.Count)							
