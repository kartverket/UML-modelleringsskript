...
'  
' Script Name: sjekkInterneKoblinger (avlegger av realiserbarGMLformat50 )
' Author: Kent Jonsrud - Section for standardization and technology development - Norwegian Mapping Authority
' Purpose: List elements that have some internal broken links.
' Date: 2021-11-16
 		...
 		case otPackage 
			...
			if not thePackage.IsModel then
				...
				'if UCase(thePackage.element.stereotype) = UCase("ApplicationSchema") then 
					...
					dim box, mess
					mess = "Lister steder der en intern kobling er brutt"&Chr(13)&Chr(10)
					mess = mess + ""&Chr(13)&Chr(10)
					mess = mess + ""&Chr(13)&Chr(10)
					mess = mess + "Starter validering av pakke [" & thePackage.Name &"]."&Chr(13)&Chr(10)

					box = Msgbox (mess, vbOKCancel, "sjekkInterneKoblinger versjon 0.4-2021-11-16")
					select case box
						case vbOK
							...
							if not abort then
								...
								Session.Output("sjekkInterneKoblinger versjon 0.4-2021-11-16 startet. "&Now())
			...
			Msgbox "Pakken [" & thePackage.Name &"] er en rotpakke. Velg en vanlig pakke."
 			end if
'			
'		case otDiagram
...
'			set theMethod = Repository.GetTreeSelectedObject()
		...
		case else
			' Error message
			Session.Prompt "[Warning] You must select a package in the Project Browser to start the validation.", promptOK 
...
sub FindInvalidElementsInPackage(package) 
	...
	if true then Session.Output("Info: package to be tested: [«" &package.element.Stereotype& "» " &package.Name& "].")
	if UCase(package.element.stereotype) = UCase("ApplicationSchema") then 
								'Check model for script breaking structures
								if scriptBreakingStructuresInModel(package) then
									...
									Session.Output("Hopper over test av pakka.")
									exit sub
								end if
								PopulatePackageDependenciesShownElementIDList(package)
